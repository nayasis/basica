...
import static io.nayasis.basica.file.Files.normalizeSeparator;
...
public class FileResourceFinder {
    ...
    private Set<File> findFiles( File rootDir, String pattern ) throws IOException {
        ...
        String fullPattern = normalizeSeparator( rootDir.getAbsolutePath() );

        if ( ! pattern.startsWith("/") ) {
            fullPattern += "/";
        }

        fullPattern = fullPattern + normalizeSeparator( pattern );
    ...
    protected void findFiles( String pattern, File dir, Set<File> result ) {
        for( File content : listDirectory(dir) ) {
            String currPath =  normalizeSeparator( content.getAbsolutePath() );
